...
// place resources and powerplants
// create the deck and the mat
// define a pile of functions that will be needed (selectZone, auction, bid, etc.)
...
// define PowergridPlayer as a subclass of Player
export class PowergridPlayer extends Player<Powergrid, PowergridPlayer> {
  score: number = 0;
  // elektroPieces: Elektro[]; // now each player has an array of Elektro pieces
  elektro: number = 50; // elektroPieces.length;  // elektro were formerly just a property of each player. Now they are a count
  cities: number = 0;  // each player begins with zero powered cities
  passedThisAuction: boolean = false;
  havePassedAuctionPhase: boolean = false;
  citiesPowered: number = 0;  // each player begins having powered zero cities on their last turn
...
// define "Powergrid" as a subclass of Game, and its functions
console.log("defining class Powergrid in index.ts");
...
// sort player order by card cost
  sortPlayers(direction: 'asc' | 'desc') {
    this.game.players.sortBy([
      'citiesPowered',  // sort player order by number of cities powered, secondarily by max plant.
      player => player.allMy(Card).max('cost') || 0
    ], direction);
  }

// rule for discarding the lowest power plant from the auction
  applyMinimumRule() {
    for (const card of $.powerplants.all(Card)) {
      if (card.cost <= this.game.players.max('score')) { // discard if of lower cost than the largest number of cities any player has ***RULE CHECK***
...
} // this is the end of the definition of the PowerGrid class

// define the "Card" as a subclass of Piece and its functions
export class Card extends Piece<Powergrid> {
  ...
  spaceFor(resource: ResourceType) {
    if (this.resourceType === resource || (this.resourceType === 'hybrid' && ['oil', 'coal'].includes(resource))) {
      return this.resources! * 2 - this.all(Resource).length;  // power plants can hold up to double what they consume
    }
    return 0;
  }
...
} // this is the end of the definition of the Card class

// define Resource as a subclass of Piece
export class Resource extends Piece<Powergrid> {
  type: ResourceType;
}

// define ResourceSpace as a subclass of Space
export class ResourceSpace extends Space<Powergrid> {
  resource: ResourceType;
  cost: number;
}

// define the City as a subclass of Space, and its functions
export class City extends Space<Powergrid> {
  owners: PowergridPlayer[] = [];
  zone: string

  costToBuild() {  // an elegant function that determines the cheapest path to each city for this player
  ...
  canBuild() { // return a true or false if this player can build in this city
...
}  // this is the end of the definition of the City class

// define PlayerMat as a subClass of Space. The playerMat displays the player's powerplants and resources
export class PlayerMat extends Space<Powergrid> {
  player: PowergridPlayer
}

// define ElektroMat as a subClass of Space. The elektroMat displays the player's elektro total
export class ElektroMat extends Space<Powergrid> {
    player: PowergridPlayer
}

// define Building as a subClass of Piece
export class Building extends Piece<Powergrid> {
  powered?: boolean = false;
}


// *** this is the main function defined in this file ***
// create a function that creates the game -- this occupies the rest of the file
console.log("In index.ts, line 215, about to run createGame");
...
// create the map, with cities and the links between them; assign cities to coloured zones
    console.log('About to create map. game.players is ' + game.players);
...
// a FOR loop to build the players' mats. A player's mat holds their powerplants and resources
  for (const player of game.players) {
    const mat = game.create(PlayerMat, 'player-mat', { player });
    mat.createMany(22, Building, 'building', { player }); //start with 22 buildings
    mat.onEnter(Card, c => {  // when a card enters the player's mat...
      c.auction = false
      c.discount = false
      if (c.name === 'nuclear-39' && map.name === 'germany') {
        game.message('Uranium is now no longer available'); // what is this rule?
        game.noMoreUranium = true
      }
    });
  };

    // a FOR loop to build the players' elektro-mats. A player's elektro mat will display their elektro total
    for (const player of game.players) {
        const eMat = game.create(ElektroMat, 'elektro-mat', { player });
        // eMat.blockViewFor("all-but-owner"); // this line blocks other players' view of your elektro-mat;
    };
...
// now, the defineActions block. A series of functions for "game" -- this fills the rest of the file
// It wants a comma-separated list of actions for the game, each followed by an arrow function: selectZone, auction, bid,
// passAuction, passBid, scrap, pass, build, arrangeResources, power, buyResource.
// Everything defined here also has to show up under defineFlow
// prompt is what the active player sees; description is what the other players see
  game.defineActions({
// SELECT ZONES **************************
    selectZone: () => action({
...
// BUILD **************************
    build: player => action({
      ...
      player.score = map.all(Building, {mine: true}).length; // a player's "score" is the number of buildings they've built
                                                             // and it's updated here in the build phase
      game.applyMinimumRule();
    }).message(
...
// POWER **************************
    power: player => action({ // we are passed the player
      prompt: 'Power your plants',
      description: 'powering plants',
      condition: map.has(Building, {player, powered: false})
    }).chooseOnBoard( // the player chooses a card that can fire; this becomes the contents of the variable "card"
      'card', player.allMy(Card, c => !c.powered && !!c.resourcesAvailableToPower()), {
        prompt: 'Select plant to power',
        skipIf: 'never',
      }
    ).chooseOnBoard( // the player chooses the resources to burn; these become the contents of the variable "resources"
      'resources', ({ card }) => card.resourcesAvailableToPower()!, { // only cards with resources can be chosen
        prompt: 'Select resources for power',
        number: ({ card }) => card.resources!,
        skipIf: ({ card }) => (  // this can be skipped if
          card.resourcesAvailableToPower()!.areAllEqual('type') // the resources on the card are of the same type
            || card.resourcesAvailableToPower()!.length === card.resources  // OR they are the right number ***RULE CHECK***
        )
      }
    ).chooseNumber(
        'fired',  { // the player chooses how many cities to power; this becomes "fired"
        min: 1,
            max: ({ card }) => card.power!,
            confirm: 'Power this many cities',
            initial:  ({ card }) => card.power!,
            skipIf: ({ card }) => card.power! === 1
    }).do(({ fired, card, resources }) => {
      card.powered = true; // flag the powerplant as "powered"
      if (!resources) resources = card.firstN(card.resources!, Resource); // grab an array of the resources on the card
      for (const resource of resources) resource.remove(); // remove them one by one
      for (const building of map.firstN(fired!,
          Building,
          {mine: true, powered: false}))  // grab an array of my unpowered buildings equal in size to the number of cities this card can power
          building.powered = true; // and power them
    }),
...
/*  ).do(({ card, resources }) => {
      ...
      if (!resources) resources = card.firstN(card.resources!, Resource); // grab an array of the resources on the card
      for (const resource of resources) resource.remove(); // remove them one by one
      for (const building of map.firstN(card.power!,
          Building,
          {mine: true, powered: false}))  // grab an array of my unpowered buildings equal in size to the number of cities this card can power
          building.powered = true; // and power them
  }),*/
...
// BUY RESOURCES **************************
    buyResource: player => action({
      ...
      validate: ({ coal, oil, garbage, uranium }) => {
        ...
        if (costOf('coal', coal) +
            costOf('oil', oil) +
            costOf('garbage', garbage) +
            costOf('uranium', uranium) > player.elektro) return 'Insufficient Elektro';
      },
      confirm: [
        `Pay {{cost}} Elektro`,
...
// define the flow of the game
// Note that game.defineFlow takes an arbitrary number of arguments, all flowSteps.  A flowStep is either
       // 1) a plain function that accepts FlowArguments (an object containing the current values of any loops or actions that the game is in the middle of)
       // 2) one of the Game#flowCommands. The Game#flowCommands that we see in this loop are:whileLoop and eachPlayer.
  game.defineFlow(
  // first argument to defineFlow: an arrow function that removes cards from the deck based on the number of players
    () => {
      // shuffle the deck and remove powerplants as needed
      deck.shuffle();
      deck.topN(8, Card, card => card.cost <= 15).putInto(powerplants);

      let removals = 0;
      if (game.players.length === 4) removals = 1;  // make these removals from the deck be <= 15
      if (game.players.length === 3) removals = 2;
      if (game.players.length === 2) removals = 1;
        let outcasts = [];
        outcasts = deck.topN(removals, Card, card => card.cost <= 15);
      game.message('Casting out: ' + outcasts);
      outcasts.remove()

      removals = 0;
      if (game.players.length === 4) removals = 3; // make these removals from the deck be > 15, and not the step-3 card
      if (game.players.length === 3) removals = 6;
      if (game.players.length === 2) removals = 5;
        outcasts = deck.topN(removals, Card, card => !!card.resourceType && card.cost > 15);
        game.message('Casting out: ' + outcasts);
        outcasts.remove();

      deck.top(Card, card => card.cost <= 15)!.putInto(deck); // if the top card of the deck is > 15, put it into the deck
      deck.first(Card, 'step-3')?.putInto(deck, {fromBottom: 0});  // put the step-3 card on the bottom of the deck
  ...
  //second argument to defineFlow: the game#flowCommand "loop" . This will run a loop of the game steps, in order
    loop(
        // note that "flow()" takes as arguments an arbitrary number of comma-separated flowSteps. A flowStep is either
        // 1) a plain function that accepts FlowArguments (an object containing the current values of any loops or actions that the game is in the middle of)
        // 2) one of the Game#flowCommands. The Game#flowCommands that we see in this loop are:whileLoop and eachPlayer.
// Here's the zeroth argument to "loop:" an arrow function that sets the zones.
        () => {
            game.message('****During testing, the game is hard-coded to use all six zones.****');
            game.zones.push('green');
            game.zones.push('purple');
            game.zones.push('yellow');
            game.zones.push('blue');
            game.zones.push('red');
            game.zones.push('brown');
        },
// here's the first flowStep argument to "loop:" whileLoop
// This while loop always gets run, but the "do" block only runs in turn 1
        whileLoop({
// I think the flaw in zone picking happens here.
// If the number of zones to play with is too low (i.e., on the first turn), select zones.
        while: () => game.zones.length < Math.min(5, Math.max(3, game.players.length)), // repeat the loop until the number of zones picked is the number of players (min of 3)
        do: playerActions({
          name: 'selectZone',
...
// do the auction
// here's the second argument to "loop:" an arrow function about auctions
        () => {
            console.log("in the main LOOP in index.ts, about to do the auction");
            game.sortPlayers('desc');
        game.message("Players sorted in descending order" + game.players);
        game.phase = 'auction';
        game.turn += 1;
...
// third argument to "loop:" eachPlayer (a flowStep) about auctions
      eachPlayer({
        name: 'auctionPlayer',
...
// buy resources
// here's the fourth argument to "loop:" an arrow function that puts us in the resource buying phase.
// It looks for Step 3 and unpowers the cities
      () => {
        if (game.turn === 1) game.sortPlayers('desc'); // resort only for first turn
        game.players.reverse();
        game.phase = 'resources';
        const discount = powerplants.first(Card, { discount: true }); // discount the lowest priced powerplant
        if (discount) {
          game.addDelay();
          discount.remove();
          deck.top(Card)?.putInto(powerplants);
        ...
        // flag all powered buildings as unpowered
        for (const building of map.all(Building, { powered: true })) building.powered = false;
      },

// fifth argument to "loop:" eachPlayer (a flowStep): purchase resources
      eachPlayer({
        name: 'purchasePlayer',
        do: playerActions({
          name: 'purchaseResources',
          optional: 'Buy no more resources this turn',
          actions: [
...
// here's the sixth argument to "loop:" an arrow function that flags us as being now in the build phase
      () => { game.phase = 'build' },

// seventh argument to "loop:" eachPlayer (a flowStep): build cities
// build into cities
      eachPlayer({
        name: 'buildPlayer',
        do: playerActions({
...
// here's the eighth argument to "loop:" an arrow function that puts us in the power-cities phase
      () => {
        game.phase = 'power';
        if (game.players.max('score') >= game.victory()) {  // if any player has built enough power plants to trigger
                                                                 // the final round
          game.message("Final power phase!");
        }
      },

// ninth argument to "loop:" eachPlayer (a flowStep) : arrange resources and power plants
// arrange and burn resources
      eachPlayer({
        name: 'powerPlayer',  // during this flowStep, "powerPlayer" is the player we are considering
        do: [
          loop(playerActions({
            name: 'arrange',
            actions: [
              'arrangeResources',
              { name: 'pass', do: Do.break } // jump out of this little loop if they decline to re-arrange resources
            ]
          })),
          loop(playerActions({
            name: 'power',
            actions: [
              'power',  // do the POWER action up at line 556
              { name: 'pass', do: Do.break } // jump out of this little loop if they decline to power anything
            ]
          })),
          ({ powerPlayer }) => {
            // count power from plants and number of cities that can be powered
            powerPlayer.cities = Math.min( // "cities" is the lesser of: the number of buildings for which powered = true, and 20
              powerPlayer.allMy(Building, { powered: true }).length,
              game.income.length - 1,
            )

//distribute income
            if (game.players.max('score') < game.victory()) {  // as long as no one's number of plants has reached the win-threshhold
              const rev = game.income[powerPlayer.cities]; // determine revenue based on the "player's "cities"
              ...
              powerPlayer.citiesPowered = powerPlayer.cities; // remember how many cities this payer powered for when we calculate turn order

              powerPlayer.cities = 0;  //immediately after distributing income, reset cities powered to 0
              // unpower plants
...
// here's the tenth argument to "loop:" an arrow function
// decide if we have moved to a new step
      () => {
        if (game.step === 1 && game.players.max('score') >= game.step2Score()) {  // if any player has built enough plants to trigger step 2
...
// If a player has won, exit the loop
        if (game.players.max('score') >= game.victory()) {
          ...
          game.finish(winner); 
// otherwise refill resources, drop the lowest numbered powerplant and check for step 3
        } else {
  ...
  );  // end of game.defineFlow

// this is the loop we follow during an auction
  game.defineSubflow(
    'bidding-round',
